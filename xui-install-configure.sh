...
# Генерируем случайный порт для x-ui web panel между 1024 and 65535
PORT_WEB=$((RANDOM % (65535 - 1024 + 1) + 1024))
...
dpkg --configure -a
DEBIAN_FRONTEND=noninteractive apt upgrade -y
DEBIAN_FRONTEND=noninteractive apt install dnsutils curl openssl ca-certificates certbot knockd iptables iptables-persistent unattended-upgrades -y
...
# Включаем BBR
(echo 24; echo 1; echo 0; echo 0) | x-ui
